...
static unsigned textsize,codesize;
static unsigned char *iptr,*optr;
static int pbbuf,pbmask,gbbuf,gbmask;
...
static void FlushBitBuffer(void) {
  *optr++=pbbuf;
  codesize++;
}
...
static void lza_init(void) {
  pbbuf=0;pbmask=128;
  gbmask=0;
  low=0;high=Q4;value=0;shifts=0;
}

unsigned lza_pack(void *ptr,unsigned sz,void *buf)
{
...
//        textsize = sz;
        ...
        if(sz == 0) return 0;
        textsize = 0;
        iptr=(unsigned char*)ptr;
        optr=(unsigned char*)buf;
        lza_init();
        StartModel();  InitTree();
        s = 0;  r = N - F;
        for (i = s; i < r; i++) text_buf[i] = ' ';
        for (len = 0; len < F && sz; len++) {
          c=*iptr++;--sz;
          text_buf[r + len] = c;
        }
        textsize = len;
        ...
        do {
                ...
                for (i = 0; i < last_match_length && sz; i++) {
                        c=*iptr++;--sz;
